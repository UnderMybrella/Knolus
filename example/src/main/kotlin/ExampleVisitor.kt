...
class ExampleVisitor(val restrictions: KnolusTransVisitorRestrictions<*>, val parser: ExampleParser, val delegate: TransKnolusParserVisitor) : ExampleParserBaseVisitor<KorneaResult<KnolusUnion>>() {
    ...
    override fun visitFunctionCall(ctx: ExampleParser.FunctionCallContext): KorneaResult<KnolusUnion.VariableValue<KnolusLazyFunctionCall>> =
        delegate.visitFunctionCall(TransFunctionCallBlueprint(ctx))

    override fun visitFunctionCallParameter(ctx: ExampleParser.FunctionCallParameterContext): KorneaResult<KnolusUnion.FunctionParameterType> =
        delegate.visitFunctionCallParameter(TransFunctionCallParameterBlueprint(ctx))

    override fun visitMemberFunctionCall(ctx: ExampleParser.MemberFunctionCallContext): KorneaResult<KnolusUnion.VariableValue<KnolusLazyMemberFunctionCall>> =
        delegate.visitMemberFunctionCall(TransMemberFunctionCallBlueprint(ctx))

    override fun visitVariableReference(ctx: ExampleParser.VariableReferenceContext): KorneaResult<KnolusUnion.VariableValue<KnolusVariableReference>> =
        delegate.visitVariableReference(TransVariableReferenceBlueprint(ctx))

    override fun visitMemberVariableReference(ctx: ExampleParser.MemberVariableReferenceContext): KorneaResult<KnolusUnion.VariableValue<KnolusPropertyReference>> =
        delegate.visitMemberVariableReference(TransMemberVariableReferenceBlueprint(ctx))

    override fun visitVariableValue(ctx: ExampleParser.VariableValueContext): KorneaResult<KnolusUnion.VariableValue<KnolusTypedValue>> =
...
//    override fun visitExpressionOperation(ctx: ExampleParser.ExpressionOperationContext): KorneaResult<KnolusUnion> = delegate.visitExpressionOperation(TransExpressionOperationBlueprint(ctx))
    ...
    override fun visitEchoCommand(ctx: ExampleParser.EchoCommandContext): KorneaResult<KnolusUnion> = visitVariableValue(ctx.variableValue())
        .map { union -> buildFunctionCallAsVar("echo", union.value) }
}